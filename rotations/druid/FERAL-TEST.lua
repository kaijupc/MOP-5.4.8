...
local inCombat = {
	...
	--{"Rip", "spell.ready && inmelee && target.combo == 5 && player.energy >= 35 && target.debuff(Rip).duration < 2", "target"},
	...
	--{"Ferocious Bite", "spell.ready && energy >= 30 && target.combo == 5 && !target.debuff(Rip).duration > 3", "target"},
	...
	--{"Rake", "spell.ready && inmelee && target.debuff(Rake).duration < 2 && !target.combo == 5", "target"},
