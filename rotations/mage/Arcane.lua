...
local exeOnLoad = function()
	...
	function _A.castsince(idd, delay)
		if delay == nil then return false end
		if _A.casttimers[idd]==nil then return false end
		return (_A.GetTime() - _A.casttimers[idd])<=delay
	end
	...
	FakeUnits:Add('lowestEnemyInSpellRange', function(num, spell)
		local tempTable = {}
		local target = Object("target")
	...
	FakeUnits:Add('lowestEnemyFrozen', function(num, spell)
		local tempTable = {}
		local player = player or Object("player")
		for _, Obj in pairs(_A.OM:Get('Enemy')) do
	...
	FakeUnits:Add('lowestEnemyNONFrozen', function(num, spell)
		local tempTable = {}
		local player = player or Object("player")
	...
	FakeUnits:Add('lowestEnemyInRangeNONFrozen', function(num, range_target)
		local tempTable = {}
	...
	FakeUnits:Add('lowestEnemyInRange', function(num, range_target)
		local tempTable = {}
		local range, target = _A.StrExplode(range_target)
		range = tonumber(range) or 40
		target = target or "player"
...
arcane.rot = {
	...
	activetrinket = function()
		if player:buff("Surge of Dominance") and player:DebuffStack("Arcane Charge")>=4 then
			for i=1, #usableitems do
				if GetItemSpell(select(1, GetInventoryItemID("player", usableitems[i])))~= nil then
					if GetItemSpell(select(1, GetInventoryItemID("player", usableitems[i])))~="PvP Trinket" then
						if cditemRemains(GetInventoryItemID("player", usableitems[i]))<.3 then 
							local lowestmelee =  Object("lowestEnemyInSpellRange(Ice Lance)")
							if lowestmelee then
								_A.CallWowApi("RunMacroText", (string.format(("/use %s "), usableitems[i])))
								if  player:SpellCooldown("Arcane Power")==0 then
									player:cast("Arcane Power")
								end
								if _A.castdelay(108978,6) then
									player:cast(108978)
								end
							end
						end
	...
	altertimeSecond = function()
		if _A.castsince(108978,6) and player:SpellCooldown("Alter Time")<.3 and player:DebuffStack("Arcane Charge")<4 then 
			return player:cast("Alter Time") 
		end
	end,
	...
	magedot = function()
		if player:SpellCooldown("Living Bomb")<.3 then
			local lowestmelee = Object("lowestEnemyInSpellRange(Ice Lance)")
			if lowestmelee then
	...
	arcaneblast_fill = function()
		--if player:keybind("E") then
		if not player:Moving() and player:SpellCooldown("Arcane Blast")<.3 then
			local lowestmelee = Object("lowestEnemyInSpellRange(Ice Lance)")
			if lowestmelee  then
				return lowestmelee:Cast("Arcane Blast")
			end
		end
		--end
	end,
	...
	arcanemissiles = function()
		--if player:keybind("E") then
		if not player:Moving() and player:SpellUsable("Arcane Missiles") and player:SpellCooldown("Arcane Missiles")<.3
			-- and player:DebuffStack("Arcane Charge")<4
			then
			local lowestmelee = Object("lowestEnemyInSpellRange(Ice Lance)")
			if lowestmelee  then
				return lowestmelee:Cast("Arcane Missiles")
			end
		end
		--end
	end,
	...
	icelance = function()
		if player:SpellCooldown("Ice Lance")<.3 then
			local lowestmelee = Object("lowestEnemyInSpellRange(Ice Lance)")
			if lowestmelee then
				return lowestmelee:Cast("Ice Lance")
			end
	...
	arcaneexplosion = function()
		if player:SpellCooldown("Arcane Explosion")<.3 then
			local lowestmelee = Object("lowestEnemyInRange(8)")
			if lowestmelee then
				return lowestmelee:cast("Arcane Explosion")
			end
		end
	end,
	
	arcanebarrage = function()
		if player:DebuffStack("Arcane Charge")>=4 and player:SpellCooldown("Arcane Barrage")<.3 then
			local lowestmelee = Object("lowestEnemyInSpellRange(Ice Lance)")
			if lowestmelee then
				return lowestmelee:cast("Arcane Barrage")
			end
		end
	end,
...
local inCombat = function()	
	...
	arcane.rot.Silencing()
	arcane.rot.icebarrier()
	arcane.rot.incanters()
	-- add deep freeze to the om temptable 
	arcane.rot.magedot()
	arcane.rot.activetrinket()
	-- arcane.rot.altertimeSecond()
	arcane.rot.arcanebarrage()
	arcane.rot.arcanemissiles()
	arcane.rot.arcaneexplosion()
	arcane.rot.arcaneblast_fill()
	arcane.rot.icelance()
end
